...
import json from '../../json/english.json' //getting the jso data
...
const Englishdocs = () => {
    ...
    return (
        <div className='English-docs-wrapper mt-[3%]  px-10 lg:px-32 py-10'>
                ...
                <div className="description-alphabets text-base font-normal mt-3">
                    ...
                    <div className='mt-3 flex justify-start flex-wrap gap-4'>
                        {json.Alphabet.Vowels.List.map((vowel, i) => {
                            return <motion.div initial={{ x: 10,  opacity: 0 }}
                                animate={{ x: 0, y: 0, opacity: 1 }}
                                transition={{ duration: 1, delay: i * 0.5 }}
                                className='vowel-wrapper bg-slate w-fit px-6 py-2 rounded-3xl text-white cursor-pointer'
                                onClick={() => {
                                    speak(vowel.split(" ")[0], 'hi-IN', 0.5)
                                }}>
                                {vowel}
                            </motion.div>
                        })}
                    ...
                    <div className='mt-3 flex justify-start flex-wrap gap-4'>
                        {json.Alphabet.Consonants.List.map((consonants, i) => {
                            return <motion.div
                                ...
                                className='consonants-wrapper cursor-pointer bg-sub px-6 py-2 rounded-3xl text-white'
                                onClick={() => {
                                    speak(consonants.split(" ")[0], 'hi-IN', 0.5)
                                }}>
                                {consonants}
                            </motion.div>
                        })}
                ...
                <div className='flex flex-wrap justify-start gap-4 mt-5'>
                    {json['Common Phrases'].List.map((phrase, i) => {
                        return <motion.div
                            ...
                            className='phrases-wrapper cursor-pointer bg-text_start w-fit px-8 py-2 rounded-3xl text-white text-base font-normal'
                            onClick={() => {
            ...
            <motion.div initial={{ y: 0, opacity: 0 }} animate={{ y: -30, opacity: 1 }} transition={{ duration: 1, delay: 1.3 }} className="text-4xl font-bold mt-14">
                कुछ सामान्य शब्दों को अंग्रेज़ी में बोलने का प्रयास करें 🔈
                <div className='flex flex-wrap justify-start gap-4 mt-5'>
                    {json.Vocabulary.map((voc, i) => {
                        return <motion.div
                            ...
                            animate={{ x: 0, y: 0, opacity: 1 }}
                            transition={{ duration: 1, delay: i * 0.2 }}
                            className='phrases-wrapper cursor-pointer bg-purple w-fit px-6 py-2 rounded-3xl text-white text-base font-normal'
                            onClick={() => {
                ...
                <motion.div className='flex flex-wrap justify-start gap-4 mt-5'>
                    {json.Practice.Sentences.map((sentence, i) => {
                        return <div
                            ...
                            className='phrases-wrapper cursor-pointer bg-indigo w-fit px-6 py-2 rounded-3xl text-white text-base font-normal'
                            onClick={() => {
